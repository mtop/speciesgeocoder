...
#polygonedges
postscript(file = "bm_no_graph_polygonedges.eps", paper = "special", width = 7.5, height = 5.5)
par(mar = c(4,4,4,4))
plot(log10(datR_simp$datapoints), log10(datR_simp$seconds), type = "n", 
     xlab = "Number of datapoints (log10)", ylab = "Time [sec]", axes = F, xlim = c(0,8))
...
points(log10(subset(datR_simp,datR_simp$polygonedges == 4)$datapoints),
       log10(subset(datR_simp,datR_simp$polygonedges == 4)$seconds), type = "b", lty = 2, pch = 2)

points(log10(subset(datR_simp,datR_simp$polygonedges == 40)$datapoints),
       log10(subset(datR_simp,datR_simp$polygonedges == 40)$seconds), type = "b", lty = 3, pch = 3)

points(log10(subset(datR_simp,datR_simp$polygonedges == 400)$datapoints),
       log10(subset(datR_simp,datR_simp$polygonedges == 400)$seconds), type = "b", lty = 4, pch = 4)

points(log10(subset(datR_simp,datR_simp$polygonedges == 4000)$datapoints),
       log10(subset(datR_simp,datR_simp$polygonedges == 4000)$seconds), type = "b", lty = 5, pch = 5)

points(log10(subset(datR_simp,datR_simp$polygonedges == 40000)$datapoints),
       log10(subset(datR_simp,datR_simp$polygonedges == 40000)$seconds), type = "b", lty = 6, pch = 1)

legend("bottomright",legend = c(4, 40, 400, 4000, 40000), 
       pch = c(2,3,4,5,1), lty="99", title = "No of polygonedges")
box("plot")
dev.off()

#with graphics
postscript(file = "bm_graphics_polygonedges.eps", paper = "special", width = 7.5, height = 5.5)
par(mar = c(4,4,4,4))
plot(log10(datR_graph$datapoints), log10(datR_graph$seconds), type = "n", 
     xlab = "Number of datapoints (log10)", ylab = "Time [sec]", axes = F, xlim = c(0,8))
...
points(log10(subset(datR_graph,datR_graph$polygonedges == 4)$datapoints),
       log10(subset(datR_graph,datR_graph$polygonedges == 4)$seconds), type = "b", lty = 2, pch = 2)

points(log10(subset(datR_graph,datR_graph$polygonedges == 40)$datapoints),
       log10(subset(datR_graph,datR_graph$polygonedges == 40)$seconds), type = "b", lty = 3, pch = 3)

points(log10(subset(datR_graph,datR_graph$polygonedges == 400)$datapoints),
       log10(subset(datR_graph,datR_graph$polygonedges == 400)$seconds), type = "b", lty = 4, pch = 4)

points(log10(subset(datR_graph,datR_graph$polygonedges == 4000)$datapoints),
       log10(subset(datR_graph,datR_graph$polygonedges == 4000)$seconds), type = "b", lty = 5, pch = 5)

points(log10(subset(datR_graph,datR_graph$polygonedges == 40000)$datapoints),
       log10(subset(datR_graph,datR_graph$polygonedges == 40000)$seconds), type = "b", lty = 6, pch = 1)
